...
Nx = 101            # 101 cell in x direction   {  assert: typeof( (Nx-1) / (Nx_coloc-1) ) == Int  }
...
t = (0:Nt)*Δx|>collect        # array of x coordinates
...
tf_sw_real, tf_p_real = imseq(tf_param_real) #  Nt x Nz x Nx
...
Nx_coloc = 51  # number of colocation points on x-axis       # 101 cell in x direction   {  assert: typeof( (Nx-1) / (Nx_coloc-1) ) == Int  }
Nt_coloc = 101  # number of colocation points on time-axis   # number of timesteps       {  assert: typeof( (Nt) / (Nt_coloc-1) ) == Int  }
...
x_dimless = x / (Nx*Δx)                         # dimensionless spatial coordinates
t_dimless = t / (ϕ[1, 1] * Nx*Δx) * 3*flow/Δx   # dimensionless time
...
loss, λ_1, λ_2, λ_3 = get_loss(tf_param_NN,
                                ...
                                sw_xx_nn,
                                f_w_x_nn,
                                false, false, false, # no calibration
                                false) # excluding production data in train data
