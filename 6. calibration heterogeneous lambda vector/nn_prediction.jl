...
Nx_coloc = 101  # number of colocation points on x-axis
Nt_coloc = 51  # number of colocation points on time-axis
...
# loss function
loss, λ_1, λ_2 = get_loss(tf_param_NN,
                                sw_nn,
                                sw_t_nn,
                                sw_xx_nn,
                                f_w_x_nn,
                                true, true, # calibrating λ1 and λ2, not calibrating λ3
...
opt = ScipyOptimizerInterface(loss; method="L-BFGS-B", options=Dict("maxiter"=> 3000, 
                                                                    "maxfun"=> 3000, 
                                                                    "ftol"=>1e-12,
                                                                    "gtol"=>1e-12))
...
loss_final, λ1, λ2 = run(sess, [loss, λ_1, λ_2])
...
# plot lambda 
close("all")
Plots.plot((1:Nx_coloc)*interpol_x*Δt, λ1, label="λ1")
Plots.plot!((1:Nx_coloc)*interpol_x*Δt, λ2, label="λ2")
name = "./results/lamda.png"
Plots.savefig(name)
