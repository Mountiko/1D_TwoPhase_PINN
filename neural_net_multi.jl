...
function get_loss(args, sw_pred, sw_t_pred, sw_xx_pred, f_w_x_pred, prod)
    #= 
    ...
    # define first calibration parameter
    λ_1 = Variable(ones(args.Nx))
    λ_1_multi = repeat(λ_1, args.Nt)

    # define second calibration parameter
    λ_2 = Variable(ones(args.Nx))
    λ_2_multi = repeat(λ_2, args.Nt)
    ...
    # PDE regularization
    loss_PDE = sum((sw_t_pred * λ_1_multi + f_w_x_pred * λ_2_multi - ϵ * sw_xx_pred)^2) / (args.Nx * args.Nt)
